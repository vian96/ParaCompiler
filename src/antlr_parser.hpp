...
protected:
  ...
  antlrcpp::Any visitProgram(ParaCLParser::ProgramContext *ctx) override {
    ...
    for (auto *stmtCtx : ctx->statement()) {
        auto stmt = take<AST::Statement>(visit(stmtCtx));
        if(stmt) node->statements.push_back(std::move(stmt));
    }
    return static_cast<AST::Node*>(node);
  }

  // --- Statement (Manual Dispatch because we removed labels) ---
  antlrcpp::Any visitStatement(ParaCLParser::StatementContext *ctx) override {
    ...
    // 4. Expression Statement (e.g. "1+1;")
    if (ctx->expr()) {
  ...
  antlrcpp::Any visitVarDecl(ParaCLParser::VarDeclContext *ctx) override {
      ...
      if (ctx->typeSpec()) {
          node->typeSpec = take<AST::TypeSpec>(visit(ctx->typeSpec()));
      }
  ...
  // --- Expressions (Labels used here, so specific functions exist) ---

  antlrcpp::Any visitIntExpr(ParaCLParser::IntExprContext *ctx) override {
      auto* node = new AST::IntLit();
      try { node->val = std::stoll(ctx->INT()->getText()); } catch(...) { node->val = 0; }
      return static_cast<AST::Node*>(node);
  }
